...
2026-06-07 追加更新：GitHub 仓库默认分支确认是 `main`，此前 README 图片只推到了 `master`，所以仓库首页仍显示旧 README。现已在 `main` 分支同步两张 README 图片资产和 HTML `<img>` 内嵌展示区块，目标是让 GitHub 首页直接渲染图片。
...
## 最近验证基线
- 最近一次全量后端回归：`py -3.12 -m pytest -q`，`198 passed`。
